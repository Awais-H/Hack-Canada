  ...
  (function() {
   var loadPackage = function(metadata) {
      ...
      if (typeof window === 'object') {
        var encodePath = (typeof encodeURIComponent === 'function')
          ? encodeURIComponent
          : (typeof window['encodeURIComponent'] === 'function' ? window['encodeURIComponent'] : null);
        if (!encodePath) {
          throw new Error('encodeURIComponent is unavailable in this extension context');
        }
        PACKAGE_PATH = encodePath(window.location.pathname.toString().substring(0, window.location.pathname.toString().lastIndexOf('/')) + '/');
        ...
        if (typeof fetch === 'function') {
          fetch(packageName)
            .then(function(response) {
              if (!response.ok) {
                throw new Error(response.status + " : " + packageName);
              }
